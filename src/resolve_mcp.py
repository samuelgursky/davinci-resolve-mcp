    ...
    try:
        if command == "next_frame":
            ...
            timeline.SetCurrentTimecode(
                timeline.GetCurrentTimecode() + 24
            ...
            if subfolders:
                for subfolder in subfolders:
                    ...
                    structure["subfolders"].append(
                        get_folder_structure(subfolder, subfolder_path)
                    )
...
def add_clip_to_timeline(
    ...
    if (
        not timeline.SetCurrentVideoTrack(track_index)
        if track_type == "video"
        else timeline.SetCurrentAudioTrack(track_index)
    ):
