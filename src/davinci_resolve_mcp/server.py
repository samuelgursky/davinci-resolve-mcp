...
def create_server():
    ...
    @mcp.tool()
    def get_clip_details(track_type: str = "video", track_index: int = 1, clip_index: int = 0) -> dict:
    ...
    def get_media_pool_items() -> list:
        ...
        try:
            ...
            for clip in clips:
                ...
                try:
                    for prop in ["FPS", "Resolution", "Format", "File Path", "Clip Color"]:
                        try:
    ...
    def control_playback(command: str = "play") -> dict:
        ...
        try:
            ...
            valid_commands = ["play", "stop", "pause", "forward", "reverse", "next_frame", 
                              "prev_frame", "next_clip", "prev_clip", "to_in", "to_out", "toggle_play"]
            
            if command not in valid_commands:
                return {"error": f"Invalid command. Valid options are: {', '.join(valid_commands)}"}
    ...
    @mcp.tool()
    def add_clip_to_timeline(media_pool_item_name: str = "", track_index: int = 1, track_type: str = "video", frame_position: int = -1) -> dict:
        ...
        try:
            ...
            if not target_clip:
                return {"error": f"Could not find media pool item with name '{media_pool_item_name}'"}
    ...
    def switch_to_project(project_name: str = "") -> dict:
        ...
        try:
            ...
            if project_name not in projects:
                return {"error": f"Project '{project_name}' not found in the current folder"}
