...
def get_current_node_index():
    ...
    return {"status": "success", "node_index": node_index}
...
def set_current_node_index(index: int):
    ...
    return {"status": "success", "message": f"Current node set to index {index}"}
...
def delete_current_node():
    ...
    return {"status": "success", "message": "Deleted current node"}
...
def reset_current_node():
    ...
    return {"status": "success", "message": f"Reset node {current_node.GetNodeIndex()}"}
...
def import_lut(lut_path: str):
    ...
    if not any(lut_path.lower().endswith(ext) for ext in valid_extensions):
        return {
            "error": f"Invalid LUT file format. Supported formats: {', '.join(valid_extensions)}"
        }
...
def apply_lut_to_current_node(lut_path: str):
    ...
    try:
        ...
        if not result:
            return {
                "error": f"Failed to apply LUT to node {current_node.GetNodeIndex()}"
            }
