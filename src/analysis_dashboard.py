...
<body data-theme="studio" data-tier="operations">
    ...
    <section class="span-12 subpage" data-subpage-scope="preferences" data-subpage="paths">
        ...
        <div class="settings-grid">
          ...
          <label>Inventory exclude bins (comma separated)
            <input id="prefInventoryExcludeBins" type="text" placeholder="e.g. assets">
          </label>
  ...
  <script>
    ...
    const PREFERENCE_HELP = {
      ...
      prefInventoryLimit: 'Maximum number of clips to index during inventory walk.',
      prefInventoryExcludeBins: 'Comma-separated names of subfolders to skip entirely during inventory walk (e.g. assets).',
    ...
    function renderSetupPreferences() {
      ...
      setControlValue('prefInventoryExcludeBins', media.inventory_exclude_bins || 'assets');
    ...
    function setupPreferencePayload() {
      ...
      return {
        media_analysis: {
          ...
          inventory_limit: parseInt($('prefInventoryLimit').value) || 500,
          inventory_exclude_bins: $('prefInventoryExcludeBins').value.trim() || 'clear',
    ...
    *,
    ...
    with _RESOLVE_API_LOCK:
        ...
        truncated = _append_folder_media(
            ...
            warnings=warnings,
            limit=max_items,
            exclude_bins=exclude_bins if exclude_bins is not None else {"assets"},
        )
        project_info = {
...
class Handler(BaseHTTPRequestHandler):
    ...
    def _route_get(self) -> None:
        ...
        if path == "/api/resolve/media":
            defaults = _setup_defaults("get_defaults").get("defaults", {}).get("media_analysis", {})
            pref_limit = defaults.get("inventory_limit") or 500
            pref_exclude_str = defaults.get("inventory_exclude_bins") or "assets"
            exclude_bins = {b.strip() for b in pref_exclude_str.split(",") if b.strip()}
            self._json_etag(
