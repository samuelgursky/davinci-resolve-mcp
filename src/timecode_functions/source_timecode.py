...
def mcp_get_source_timecode_report() -> Dict[str, Any]:
    ...
    report = {"timeline_name": timeline.GetName(), "clips": []}
...
def mcp_export_source_timecode_report(
    ...
    if video_tracks_only:
        report_data["clips"] = [
            clip for clip in report_data["clips"] if clip.get("track_type") == "video"
        ]
    ...
    try:
        ...
        if format.lower() == "csv":
            ...
            with open(export_path, "w") as f:
                # Write header
                f.write(
                    "Name,Track,Timeline Start,Timeline End,Duration,Source In TC,Source Out TC,File Path\n"
                )

                # Write data
                for clip in report_data["clips"]:
                    f.write(
                        f"{clip.get('name', '')},{clip.get('track', '')},"
            ...
            with open(export_path, "w") as f:
                ...
                for i, clip in enumerate(report_data["clips"]):
                    if (
                        "timeline_source_in_tc" in clip
                        and "timeline_source_out_tc" in clip
                    ):
                        if (
                            clip.get("track_type") == "video"
                        ):  # EDL typically only includes video
                            # Write EDL entry
                            f.write(
                                f"{i+1:03d}  {clip.get('name', 'CLIP')}       V     C        "
                            )
                            f.write(
                                f"{frames_to_timecode(clip.get('start_frame', 0), 24.0)} "
                            )
                            f.write(
                                f"{frames_to_timecode(clip.get('end_frame', 0), 24.0)} "
                            )
                            f.write(
                                f"{clip.get('timeline_source_in_tc', '00:00:00:00')} "
                            )
                            f.write(
                                f"{clip.get('timeline_source_out_tc', '00:00:00:00')}\n"
                            )
            ...
            return {
                "error": f"Unsupported format: {format}. Supported formats are: csv, json, edl"
            }
