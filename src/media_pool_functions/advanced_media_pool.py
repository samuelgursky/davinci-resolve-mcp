    ...
    if include_subfolders:
        ...
        for subfolder in subfolders:
            if recursive:
                ...
                subfolder_list.append({"name": subfolder.GetName()})
...
def get_folder_by_path(
    path: str, include_clips: bool = False, include_subfolders: bool = False
):
    """
    ...
    folder_dict = _convert_folder_to_dict(
        folder, include_clips=include_clips, include_subfolders=include_subfolders
    )
...
def set_current_folder(path: str):
    ...
    return {"status": "success", "message": f"Current folder set to: {path}"}
...
def get_current_folder():
    ...
    folder_dict = _convert_folder_to_dict(
        folder, include_clips=True, include_subfolders=True
    )

    return {"status": "success", "folder": folder_dict}


def move_clips_between_folders(
    source_path: str, destination_path: str, clip_names: List[str] = None
):
    """
...
def get_smart_bins():
    ...
    for bin_name in default_bins:
        ...
        if smart_bin:
            ...
            smart_bins.append(
                {"name": bin_name, "is_default": True, "clip_count": clip_count}
            )
    ...
    return {"status": "success", "smart_bins": smart_bins}
...
def delete_smart_bin(name: str):
    ...
    try:
        ...
        if result:
            return {"status": "success", "message": f"Deleted smart bin: {name}"}
        else:
        ...
        try:
            ...
            return {
                "error": f"Failed to delete smart bin (unsupported operation): {name}"
            }


def bulk_set_clip_property(
    folder_path: str,
    property_name: str,
    property_value: str,
...
def mcp_move_clips_between_folders(
    source_path: str, destination_path: str, clip_names: List[str] = None
):
    """
...
def mcp_bulk_set_clip_property(
    folder_path: str,
    property_name: str,
    property_value: str,
    clip_names: List[str] = None,
    ...
    Returns:
        Status of the operation
    """
    return bulk_set_clip_property(
        folder_path, property_name, property_value, clip_names
    )
