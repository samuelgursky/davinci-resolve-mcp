...
def add_timeline_marker(
    frame=None, color="Blue", name="", note="", duration=1, custom_data=""
):
    """
    ...
    if color not in valid_colors:
        return {
            "error": f"Invalid marker color. Valid colors are: {', '.join(valid_colors)}"
        }

    # Create the marker
    success = timeline.AddMarker(frame, color, name, note, duration, custom_data)
...
def update_marker(
    frame, color=None, name=None, note=None, duration=None, custom_data=None
):
    """
    ...
    if not success:
        return {
            "error": f"Failed to update marker at frame {frame} (could not delete old marker)"
        }
    ...
    if success:
        ...
        return {
            "error": f"Failed to update marker at frame {frame} (could not add new marker)"
        }
...
def delete_markers_by_color(color):
    ...
    if not markers_before:
        return {
            "status": "success",
            "deleted_count": 0,
            "message": "No markers found in timeline",
        }
    ...
    if success:
        ...
        deleted_count = len(markers_before) - (
            len(markers_after) if markers_after else 0
        )

        return {"status": "success", "deleted_count": deleted_count, "color": color}
    else:
