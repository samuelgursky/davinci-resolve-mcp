...
try:
    ...
    resolve_api_path = os.environ.get('RESOLVE_SCRIPT_API', 
...
def mcp_set_project_setting(setting_name: str, setting_value: Any) -> Dict[str, Any]:
    ...
    try:
        ...
        numeric_settings = [
            ...
            "videoMonitorWidth", "videoMonitorHeight", "videoMonitorBitDepth",
            "superScale", "audioSampleRate"
        ]
        
        boolean_settings = [
            "timelineUseCustomSettings", "videoMonitorUseRec709A", "videoMonitorUseColorspaceOverride",
            "superScaleQuality", "videoMonitorUse3D", "useRollingShutter", "useSmoothOpticalFlow",
            "useDynamicZoomEase", "autoClipColors", "audoColorLuminanceMix"
        ]
        
        enum_settings = {
            "colorScienceMode": ["davinciYRGBColorManaged", "davinciYRGB", "davinciACESCC"],
            "timelineOutputSizing": ["outputSizingFormatCentered", "outputSizingFormatFill", "none"],
            "inputColorSpace": ["Rec.709", "Rec.2020", "BMD Film", "BMD 4K Film", "BMD 4.6K Film", "Panasonic V-Log"],
        ...
        if setting_name in numeric_settings:
            try:
                ...
                return {"error": f"Invalid numeric value '{setting_value}' for setting '{setting_name}'"}
            ...
            if isinstance(setting_value, bool):
                ...
                if setting_value.lower() in ["true", "yes", "1", "on"]:
                    ...
                    return {"error": f"Invalid boolean value '{setting_value}' for setting '{setting_name}'"}
            ...
            if setting_value not in valid_values:
                return {"error": f"Invalid value '{setting_value}' for setting '{setting_name}'. Valid values are: {', '.join(valid_values)}"}
            ...
            try:
                ...
                if timeline:
                    ...
                    if timeline_setting and len(timeline_setting) > 0:
                        timeline_setting = timeline_setting[0].lower() + timeline_setting[1:]
                        timeline_success = timeline.SetSetting(timeline_setting, setting_value)
                        if timeline_success:
                            return {
                                "status": "success", 
        ...
        try:
            if hasattr(current_project, "GetSetting") and callable(current_project.GetCurrentTimeline):
                # Try to get and set project format settings
                timeline = current_project.GetCurrentTimeline()
                if timeline and hasattr(timeline, "GetSetting") and callable(timeline.SetSetting):
                    # Attempt via format settings on timeline
                    if setting_name in ["timelineFrameRate", "timelineResolutionWidth", "timelineResolutionHeight"]:
        ...
        try:
            if hasattr(current_project, "LoadRenderPreset") and callable(current_project.LoadRenderPreset):
                # Some settings might be changed by loading a preset and then modifying it
                if setting_name in ["colorScienceMode", "timelineResolutionWidth", "timelineResolutionHeight", "timelineFrameRate"]:
                    ...
                    if presets and len(presets) > 0:
                        ...
                        # Try setting again
                        second_try = current_project.SetSetting(setting_name, setting_value)
                        if second_try:
                            return {
                                "status": "success", 
...
def mcp_get_timeline_setting(timeline_name: Optional[str] = None, setting_name: Optional[str] = None) -> Dict[str, Any]:
    """
...
def mcp_set_timeline_setting(setting_name: str, setting_value: Any, timeline_name: Optional[str] = None) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        # Define setting categories for proper type handling
        numeric_settings = [
            "width", "height", "frameRate", "audioSampleRate", "audioFrameRate", 
            "videoFrameRate", "timecodeRate"
        ]
        
        boolean_settings = [
            "useRollingShutter", "useSmoothOpticalFlow", "useDynamicZoomEase",
            "useSmartcache", "usePerfectResizeNE", "createStereoVersion"
        ]
        ...
        try:
            project_setting_name = "timeline" + setting_name[0].upper() + setting_name[1:]
            project_success = current_project.SetSetting(project_setting_name, setting_value)
            if project_success:
                return {
                    "status": "success", 
                ...
                if timeline_count > 1:
                    for i in range(1, timeline_count + 1):
                        ...
                        if temp_timeline and temp_timeline != timeline:
                            ...
                            # Try setting again
                            second_try = timeline.SetSetting(setting_name, setting_value)
                            if second_try:
                                return {
                                    "status": "success", 
