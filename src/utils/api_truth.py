...
API_TRUTH: List[Dict[str, Any]] = [
    ...
    },
    {
        ...
        "reality": "BUILD-DEPENDENT. On 21.0.0 the Python bridge returned a callable "
                   "for ANY attribute name, so hasattr(obj, 'TotallyMadeUpMethod') was "
                   "always True and getattr never raised, making capability detection "
                   "by hasattr impossible (hasattr reported SetStart, Razor, AddNode, "
                   "GenerateProxy, AddSmartBin etc. as present though none exist). "
                   "This NO LONGER reproduces on 21.0.2.4: a control probe of "
                   "'TotallyMadeUpMethod_xyz123' against Resolve, ProjectManager, "
                   "Project, MediaPool, Folder, MediaPoolItem, Timeline and "
                   "TimelineItem returned getattr-callable False on all eight — "
                   "matching dir() in every case. Treat the fabricating behaviour as "
                   "present on at least some builds and absent on others, and do not "
                   "rely on either.",
        "recommended": "Prefer dir(obj) membership for capability probes: it is "
                       "correct on every build measured, whereas hasattr/getattr is "
                       "correct only on some. Where hasattr IS used (server._has_method), "
                       "it is sound on 21.0.2.4 but silently over-reports on 21.0.0. "
