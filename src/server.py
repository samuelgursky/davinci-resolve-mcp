...
def _fusion_group_guardrails_note() -> Dict[str, Any]:
    return {
        "guardrails": list(FUSION_GROUP_GUARDRAILS),
        "instance_input_note": (
            "LoadSettings may not remap Edit-page InstanceInput order until the "
            "group is selected in Fusion and settings are reloaded via UI."
        ),
    }
...
def _find_fusion_group(comp, group_name: str):
    ...
    if tool:
        ...
        if attrs.get("TOOLS_RegID") == "GroupOperator":
            return tool
    tool_list = comp.GetToolList(False, "GroupOperator") or comp.GetToolList() or {}
...
def _fusion_group_settings_export(comp, p: Dict[str, Any]) -> Dict[str, Any]:
    ...
    out.update(_fusion_group_guardrails_note())
    return out


def _fusion_group_settings_patch_controls(p: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        summary = patch_setting_file(
            source_path,
            dest_path,
            template_key=p.get("template", "thought_bubble"),
            group_name=p.get("group_name"),
            max_scale=float(p.get("max_scale", 0.25)),
        )
    except Exception as exc:
...
def _fusion_group_settings_load(comp, p: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        return _err(f"LoadSettings failed: {exc}")
    hint = fusion_commit_hint(modified=True)
    return {
        "success": True,
        "group_name": group_name,
        ...
        "warning": (
            "Group was not deleted. InstanceInput remap may require selecting the "
            "group in Fusion and using UI Load Settings if Edit Controls look wrong."
        ),
        "commit_hint": hint,
        **_fusion_group_guardrails_note(),
    }
...
def _fusion_comp_bulk_set_expressions(p: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not isinstance(ops, list) or not ops:
        return _err(
            "bulk_set_expressions requires params.ops: non-empty list. "
            "Each op needs tool_name, input_name, expression, and timeline scope."
        )
    results: List[Dict[str, Any]] = []
    for index, op in enumerate(ops):
        if not isinstance(op, dict):
    ...
    out = {"results": results, "op_count": len(ops)}
    if any(row.get("success") for row in results):
        out["commit_hint"] = fusion_commit_hint(modified=True)
        out.update(_fusion_group_guardrails_note())
    return out
...
def fusion_comp(action: str, params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    Actions:
      ...
      group_settings_export(group_name, path) -> {path, published_inputs, input_count}
      group_settings_patch_controls(source_path, dest_path?, template?, max_scale?, group_name?) -> {dest_path, summary, diff}
      group_settings_load(group_name, settings_path, backup_path?) -> {backup_path, warning, commit_hint}
      bulk_set_expressions(ops) -> {results, op_count, commit_hint?} — each op: tool_name, input_name, expression + timeline scope
      probe_group_published_inputs(group_name, settings_path?, max_inputs?) -> {live_inputs, file_summary}
      fusion_commit_hint() -> {checklist, guardrails, instance_input_note}
    ...
    if action == "group_settings_patch_controls":
        return _fusion_group_settings_patch_controls(p)
    if action == "fusion_commit_hint":
        return fusion_commit_hint(modified=bool(p.get("modified", True)))
