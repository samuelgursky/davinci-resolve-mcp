...
- **Workaround / current handling:** No API workaround exists, but the style IS reachable below the API: it lives in Sm2TiTrack.FieldsBlob for Type=2 tracks, as an EffectFiltersBA payload whose effect 136 carries a Qt QFont descriptor (param 18) and a normalised position vector (param 17). Exposed as project_db list_subtitle_styles / set_subtitle_style (font family/size/weight/italic + position). Caveats: whole-TRACK style not per-caption, project must be CLOSED, and Resolve must be fully quit and relaunched afterwards. Burn-in overlays via Fusion titles remain a visual alternative but do not produce subtitle tracks.
