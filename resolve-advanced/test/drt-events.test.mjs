...
const CURVE = '0000000100000007000000080059004d0069006e0000000600bff0000000';
...
test('parseDRT reads the real clip/transition fields and the pool sequence encodings (E139)', async () => {
  ...
  assert.deepEqual(v1.map((c) => [c.name, c.in, c.timemap, c.mediaFrameRate]), [
    ['Universal Counting Leader', 72, 'linear', 24], ['A.mov', 3159, 'linear', 24], ['B.mov', 500, 'linear', 24], ['C.mov', 10, 'curve', 24],
  ]);
  ...
  assert.equal(a1.timemap, 'linear-multi');
});
...
test('drtEventsFromParsed walks one timeline into sequence-relative events with transitions, flags and lanes (E139)', async () => {
  ...
  assert.deepEqual(rows, [
    ...
    ['V', 'C.mov', 10, null, 456, 504, null],
    ['V2', 'D.mov', 0, 33, 92, 125, 100],
    ['V2', 'BL', 0, 0, 92, 92, 100],
    ['A', 'Mix.wav', 0, 600, 0, 600, 100],
  ]);
  const leader = r.events[0], b = r.events[2], c = r.events[3], d = r.events[4], mix = r.events[6];
  ...
  // a keyed curve is a retime this reader does not decode — never faked to 100%
  assert.equal(c.retimeUnknown, true);
...
test('two DRT versions diff through turnover_changelist: identical, a trim, a dropped dissolve (E139)', async () => {
  ...
  assert.equal(same.retained, 6, 'six cuts: the BL fade leg is a carrier, not a cut');
...
test('drtEventsFromParsed picks by name or index and refuses a name it cannot find (E139)', async () => {
  const parsed = await parseDRT(await buildDrt());
  assert.equal(drtEventsFromParsed(parsed, { timeline: 'REEL_TEST' }).events.length, 7);
  assert.equal(drtEventsFromParsed(parsed, { timeline: 0 }).events.length, 7);
