...
| An **audio** item's `source_start` reads back in the **media's** frame rate — 24 fps for a WAV | Studio 21.0.3.7 (2026-08-09) | `probe_timeline_structure` returned `source_start: 56871` for a WAV on a 29.97 fps timeline. Read at 29.97 that is 1897.6 s; the real offset is 56871/24 = 2369.6 s — out by 7m52s. `source_end` is derived as `source_start + timeline_duration`, so the pair stays self-consistent and nothing looks wrong | Convert audio source frames with the media's rate (`/ 24` for a WAV), never the timeline's; take the rate from the clip's `FPS` property or ffprobe. Video items are fine — they report in their own, matching rate. Feed the same media-rate numbers back to `create_variant_from_ranges`; it converts on placement and reports it in `duration_delta` |
